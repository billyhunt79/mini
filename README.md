...
- May 8, 2026 (latest): **"Be agentic on every model" pass — explore-first prompt + qwen overlay + runtime auto-nudge.** Three layered defenses so any model (qwen2.5 / kimi / smaller llamas) explores a directory path instead of asking the user to name a file: `default.md` "Investigate Before Asking" section + softened Stop Conditions, new `prompts/overlays/qwen.md` (cites Qwen function-calling guide), and an `agent.py` one-shot auto-nudge bounded to one retry per user turn. 13 new pytest cases, 89 prompt + 196 targeted regression tests pass, zero regressions. Branch: `fix/agentic-on-every-model`.
